...
{
    asm volatile("nop \n nop \n nop");
    gpio_put(BME_PIN_CS, 0); // Active low
    asm volatile("nop \n nop \n nop");
}
...
static void write_register(uint8_t reg, uint8_t data)
{
    ...
    buf[1] = data;
    cs_select();
    spi_write_blocking(BME_SPI_PORT, buf, 2);
    cs_deselect();
    sleep_ms(10);
}
...
static void read_registers(uint8_t reg, uint8_t *buf, uint16_t len)
{
    ...
    reg |= READ_BIT;
    cs_select();
    spi_write_blocking(BME_SPI_PORT, &reg, 1);
    sleep_ms(10);
    spi_read_blocking(BME_SPI_PORT, 0, buf, len);
    cs_deselect();
    sleep_ms(10);
}
...
{
    // Use SPI0 at 0.5MHz.
    spi_init(BME_SPI_PORT, 500 * 1000);
    gpio_set_function(BME_PIN_MISO, GPIO_FUNC_SPI);
    gpio_set_function(BME_PIN_SCK, GPIO_FUNC_SPI);
    gpio_set_function(BME_PIN_MOSI, GPIO_FUNC_SPI);

    // Chip select is active-low, so we'll initialise it to a driven-high state
    gpio_init(BME_PIN_CS);
    gpio_set_dir(BME_PIN_CS, GPIO_OUT);
    gpio_put(BME_PIN_CS, 1);
